...
In order to to reduce their size so they can be included in the package, we have additionally selected and transformed the original raster files, grouping them using the `terra::aggregate()` function with a factor of 100. The result is the following folders:
...
By default, only the satellite bands (*B01* to *B12*) are considered. By setting the `only_bands` parameter to `FALSE`, we can indicate that all raster files included in the folders that cover the area of interest are also treated, as shown in the code below.
